...
class g_screen():
		...
		for y in range(start,len(self.hit_matrix)):
			for x in range(start,len(self.hit_matrix[0])):
				...
				if self.hit_matrix[y][x] == 1:
					...
					s.blit(gra_files.gdic['display'][58],(((x-start)*32)+plusx,(y-start)*32+plusy))
	...
	def render_text(self,text,replace_keys=True,more_txt=False):
...
class maP():
			...
			try:
				for ty in range(pos[1]-2,pos[1]+3):
					for tx in range(pos[0]-2,pos[0]+3):
						if self.tilemap[ty][tx].techID == tl.tlist['misc'][16]:
							test = False
						elif self.tilemap[ty][tx].techID == tl.tlist['mine'][0]:
							test = False
						elif self.tilemap[ty][tx].techID == tl.tlist['dungeon'][3]:
							test = False
						elif self.tilemap[ty][tx].techID == tl.tlist['misc'][12]:
							test = False
			except:
				test = False
	...
	def monster_die(self,x,y,xp = None):
		...
		if self.npcs[y][x].move_border < 10:
			screen.write_hit_matrix(x,y,13)
	...
	def lvl_up(self):
		...
		while run:
			...
			if c == 0:
				...
				run = False
			elif c == 4 and self.attribute.max_lp < 20:
				self.attribute.max_lp += 1
				run = False
			elif c == 4 and self.attribute.max_lp >= 20:
				t = screen.render_text(texts['Strength'],more_txt=True)
				t = screen.render_text(texts['Skill'],more_txt=True)
				t = screen.render_text(texts['Power'],more_txt=True)
				if self.attribute.max_lp < 20:
					t = screen.render_text(texts['Will'],more_txt=True)
					screen.render_text(texts['Health'])
				else:
...
class time_class():
	...
	def tick(self):
		...
		if self.minute % 5 == 0:#every 5th turn
			...
			if player.buffs.bleeding <= 0 and player.buffs.poisoned <= 0 and player.lp < player.attribute.max_lp and player.lp >= player.last_lp:
				player.lp += 1
